...
class DataProcessingController extends Controller
{
                ...
                if ($existingStudentId !== null) {
                    ...
                    if (count($existingLesson) < 1) {
                        Lesson::create([
                            ...
                            'start' => $this->getTimeValue($lesson['lesson_time']),
                            'end' => date('H:i', strtotime($this->getTimeValue($lesson['lesson_time']) . ' +20 minutes')),
                            'day' => $lesson['lesson_day'] ?: null,
    ...
    public function getTimeValue($lessonTime){
        if($lessonTime != null && $lessonTime != 'Not Set'){
            return date('H:i', strtotime($lessonTime . ' +20 minutes'));
        } else {
            return null;
        }
    }
